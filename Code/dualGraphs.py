...
# additional argument ILmax added by Q.Z. 07/28/2022 to tolerate user-defined bulge/internal loops in a stem
def changeHelices(RNA, ILmax):    
    changes = []
    for i in range(1,len(RNA.Helices)-1):  
        ...
        if RNA.Helices[i].flag == 'L' and RNA.Helices[i+1].flag == 'L':
            ...
            if (abs(Total5P) <= ILmax and abs(Total3P) <= ILmax): # Q.Z. 07/28/2022 to tolerate user-defined bulge/internal loops
                changes.append(i)
...
# Q.Z. 07/28/2022 to tolerate user-defined bulge/internal loops
def main(ILmax):            
    ...
    if not adjMatTrue:
        countHelices(RNA) 
        # changeHelices(RNA)
        changeHelices(RNA, ILmax) # Q.Z. 07/28/2022 to tolerate user-defined bulge/internal loops
        RNA.makeMatrices()
        connectHelices(RNA)
